...
def load_pretrained_model(model_name, dataset_name):
    if model_name != "biggan":
        G = models.get_model(model_name,
                             f"../pretrained/{model2available_dataset[model_name][dataset_name]}")
    else:
...
def get_segmentation_model(model_name):
    if model_name == "face_bisenet":
        face_bisenet = models.get_model("face_bisenet", "../pretrained/face_bisenet.pth")
...
stylegan2_part2lelsd_model_paths = {
    "FFHQ": "eclipsed_stylegan2_ffhq",
    "LSUN Car": "eclipsed_stylegan2_lsun_car",
    "MetFaces": "eclipsed_stylegan2_metfaces",
    "LSUN Horse": "eclipsed_stylegan2_lsun_horse",
    "LSUN Church": "eclipsed_stylegan2_lsun_church",
}
...
st.title("Sequential Editing using eCLIPsed")
...
base_eclipsed_path = os.path.join(last_path, dates_path, "model.pth")
base_lelsd = eCLIPsed.load(base_eclipsed_path)
...
w_idx = 0
alpha = st.sidebar.slider(f'Alpha', float(alpha_min_value), float(alpha_max_value), 0.0, step=0.25 if (alpha_range_type == 'normal') else 1.0)
