""" eCLIPsed PyTorch model.
    By Julie Favre, Tomás Ebensperger, Kepler Warrington-Arroyo
    Derived from "Optimizing Latent Space Directions For GAN-based Local Image Editing"
...
class eCLIPsed:
    ...
    def __init__(self, device, localization_layers, semantic_parts, localization_loss_function="L2",
    ...
    def fit(self, gan_sample_generator, segmentation_model, analyzer: PromptAnalyzer, num_batches,
        ...
        analyzer: PromptAnalyzer
            An instance of an analyzer which has already processed a prompt
        num_batches: int
            Number of batches used for optimizing the latent directions
        single_image_seed: int
            If specified, trains the model on a single image, generated from the given seed
        clip_model_name: str
            Name of the CLIP model to use
        clip_loss_lambda: float
            Factor by which to multiply the clip loss
        localization_loss_lambda: float
            Factor by which to multiply the localization loss
        gamma_correlation: float
            Factor by which to multiply the correlation loss
        ...
        with torch.no_grad():
            text_token = clip.tokenize(analyzer.new_prompt).to(self.device)
            text_features = clip_model.encode_text(text_token).float().to(self.device)
            text_features = text_features / text_features.norm(dim=-1, keepdim=True)

        # Despite its signature, the CLIP preprocessing function does
        # not work with tensors, so we recreate a similar one here
        preprocess_for_clip = nn.Sequential(
            nn.Upsample(scale_factor=2 ** 3), # Upscale the input image
            nn.AdaptiveAvgPool2d(output_size=(224, 224)), # "AvgPool2d"-it back down to the CLIP input resolution
            nn.LazyBatchNorm2d()
        ).to(self.device)
        ...
        for i, seed in enumerate(pbar):
            ...
            if single_image_seed is not None:
                batch_data = gan_sample_generator.generate_batch_from_seeds(seeds=single_image_seed*np.ones(batch_size,dtype=np.int64), requires_grad=True,
            ...
            image_input = preprocess_for_clip(new_batch_data['raw_image'])
            ...
            loss = clip_loss_lambda * clip_loss + \
                   localization_loss_lambda * localization_loss + \
                   gamma_correlation * correlation_loss
            ...
            if summary:
                if global_step % snapshot_interval == 0:
                    ...
                    image_grid = torchvision.utils.make_grid(images, nrow=4 if localization_loss_lambda > 0 else 2)
                ...
                summary_writer.add_scalar("Total loss", loss, global_step)
                if clip_loss_lambda > 0:
